...
function insert(Book book) returns int|error {
    sql:ExecutionResult executionResult = check cl->execute(`
            INSERT INTO Book (title, author, bookshopName, isbn, price)
            VALUES (${book.title}, ${book.author}, ${book.bookshopName}, ${book.isbn}, ${book.price});`);
